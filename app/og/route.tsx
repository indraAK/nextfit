...
export async function GET(req: Request) {
  try {
    ...
    const { title = siteConfig.name, description, mode } = Object.fromEntries(searchParams);
    ...
    return new ImageResponse(
      (
        ...
        >
          ...
          >
            <div tw="text-5xl font-bold tracking-tight leading-tight dark:text-white px-8">{title}</div>
            {description ? (
              <div tw="mt-5 text-3xl text-slate-400 text-center font-normal tracking-tight leading-tight px-20">{description.substring(0, 100)}</div>
            ) : null}
          </div>
        </div>
      ),
